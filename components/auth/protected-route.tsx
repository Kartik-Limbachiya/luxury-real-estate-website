...
export function ProtectedRoute({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading) {
      ...
      // Logged in but profile incomplete/not verified
      if (user && (!profile || !profile.is_verified) && pathname !== '/auth/complete-profile') {
        console.log('[ProtectedRoute] User logged in but profile incomplete, redirecting to complete-profile')
        router.push('/auth/complete-profile')
        return
      }
  ...
  // Don't render if not authenticated or profile not complete
  if (!user) {
    return null
  }
