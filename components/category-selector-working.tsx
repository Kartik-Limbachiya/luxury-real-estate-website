...
const FinancialBenefits = () => (
  <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-8 mb-8">
    <h3 className="text-2xl font-bold text-green-800 mb-4 text-center flex items-center justify-center gap-2">
    ...
    {/* Additional Benefits */}
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white/50 rounded-lg p-4 border border-green-300">
      <div className="text-center">
        <div className="text-3xl font-bold text-green-700 mb-1">108</div>
        <div className="text-sm font-semibold text-green-900">Total Amenities</div>
      </div>
      <div className="text-center">
        <div className="text-2xl font-bold text-blue-700 mb-1">₹</div>
        <div className="text-sm font-semibold text-blue-900">Government Subsidies</div>
        <div className="text-xs text-blue-700">Available on Eligible Cases</div>
      </div>
      <div className="text-center">
        <div className="text-2xl font-bold text-pink-700 mb-1">👩</div>
        <div className="text-sm font-semibold text-pink-900">Female Property Registration</div>
        <div className="text-xs text-pink-700">Mother/Wife/Sister/Daughter</div>
      </div>
    </div>
  </div>
)
          ...
          <CardContent className="space-y-6">
            {selectedCategory === "Religious" && (
              <div className="space-y-6">
                {/* Mystical/Spiritual Foundation */}
                <div className="bg-gradient-to-r from-indigo-100 via-purple-100 to-pink-100 p-6 rounded-lg border-2 border-indigo-400">
                  <h6 className="font-bold text-xl mb-4 text-indigo-900 flex items-center justify-center gap-2">
                    <span className="text-2xl">✨</span>
                    Gudh Rahasyamay - Mystical Spiritual Foundation
                  </h6>
                  <p className="text-center text-sm text-indigo-800 mb-4 font-medium">
                    Based on sacred Shastra principles: Niti-Niyam, Vidhi-Vidhan
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    {[
                      { name: "Hom-Havan", desc: "Sacred Fire Rituals", icon: "🔥" },
                      { name: "Tap-Jap-Khap", desc: "Austerity, Chanting, Penance", icon: "🧘" },
                      { name: "Sacred Geometry", desc: "Square, Circle, Triangle", icon: "⬛" },
                    ].map((item, idx) => (
                      <div key={idx} className="bg-white rounded-lg p-4 border border-indigo-200 text-center">
                        <div className="text-3xl mb-2">{item.icon}</div>
                        <div className="font-bold text-indigo-900 text-sm mb-1">{item.name}</div>
                        <div className="text-xs text-indigo-700">{item.desc}</div>
                      </div>
                    ))}
                  </div>
                  <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg p-4">
                    <p className="text-center text-sm font-bold">
                      🕉️ Ancient Shastra-based planning for harmonious living
                    </p>
                  </div>
                </div>

                {/* 24 Tirthankar Naming System */}
                <div className="bg-gradient-to-r from-amber-100 via-orange-100 to-red-100 p-6 rounded-lg border-2 border-amber-400">
                  <h6 className="font-bold text-xl mb-4 text-amber-900 flex items-center justify-center gap-2">
                    <span className="text-2xl">🕉️</span>
                    24 Tirthankar Naming System
                  </h6>
                  <p className="text-center text-sm text-amber-800 mb-4 font-medium">
                    Sacred naming convention based on Tirthankar birthplaces, parents, symbols, and life stories
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {[
                      { element: "Main Gates", named: "Tirthankar Birthplace", icon: "🚪" },
                      { element: "Roads", named: "Tirthankars", icon: "🛣️" },
                      { element: "Gardens", named: "Tirthankar Mothers", icon: "🌸" },
                      { element: "Streets", named: "Tirthankar Fathers", icon: "🏘️" },
                      { element: "Incoming Roads", named: "Ganadhara Names", icon: "➡️" },
                      { element: "Outgoing Roads", named: "Yaksha Names", icon: "⬅️" },
                      { element: "Trees", named: "Vriksha (Sacred Tree)", icon: "🌳" },
                      { element: "Art Galleries", named: "Lanchhan (Symbols)", icon: "🎨" },
                      { element: "Jinalaya", named: "Moolnayak Form", icon: "🕉️" },
                      { element: "Homes", named: "Family Members", icon: "🏠" },
                    ].map((item, idx) => (
                      <div key={idx} className="bg-white rounded-lg p-3 border border-amber-200">
                        <div className="flex items-center gap-2">
                          <span className="text-xl">{item.icon}</span>
                          <div className="flex-1">
                            <div className="font-semibold text-amber-900 text-sm">{item.element}</div>
                            <div className="text-xs text-amber-700">{item.named}</div>
                          </div>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="bg-gradient-to-r from-amber-600 to-orange-600 text-white rounded-lg p-4 mt-4">
                    <p className="text-center text-sm font-bold">
            ...
            {selectedCategory === "Residential" && (
              <div className="space-y-6">
                {/* Swastik Building Architecture */}
                <div className="bg-gradient-to-r from-purple-100 via-pink-100 to-orange-100 p-6 rounded-lg border-2 border-purple-300">
                  <h6 className="font-bold text-xl mb-4 text-purple-900 flex items-center justify-center gap-2">
                    <span className="text-2xl">🕉</span>
                    SEWAS Building Architecture - Swastik Concept
                  </h6>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    {Object.entries(categoryData.Residential.buildingStructure).map(([key, value]) => (
                      <div key={key} className="bg-white rounded-lg shadow-lg p-4 text-center border border-purple-200">
                        <div className="text-2xl font-bold text-purple-700 mb-1">{value.split('=')[0]?.trim() || value}</div>
                        <div className="text-sm font-semibold text-purple-900">{key}</div>
                        {value.includes('=') && <div className="text-xs text-purple-600 mt-1">{value.split('=')[1]?.trim()}</div>}
                      </div>
                    ))}
                  </div>
                  <div className="bg-white rounded-lg p-4 border border-orange-300">
                    <p className="text-center text-sm text-orange-900 font-semibold">
                      🏢 1 Swastik = 8 Floors = 256 Homes | 4 Swastik = 1,024 Homes per City
                    </p>
                    <p className="text-center text-xs text-orange-700 mt-2">
                      Sacred architecture combining tradition with modern living
                    </p>
                  </div>
                </div>
            ...
            {selectedCategory === "Commercial" && (
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-blue-100 via-purple-100 to-pink-100 p-6 rounded-lg border-2 border-blue-300">
                  <h6 className="font-bold text-xl mb-4 text-blue-900 flex items-center justify-center gap-2">
                    🏪 Additional Commercial Services
                  </h6>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={idx} className="bg-white rounded-lg p-3 border border-blue-200 text-center">
                        <div className="font-semibold text-blue-900 text-sm">{service}</div>
                      </div>
                    ))}
                  </div>
                </div>
